...
trystr = 25
%git pull
[status, cmdout] = system('git -C ./phase-rep/ pull'); disp(cmdout);
% pause(1)
...
[~,~] = system('ffmpeg -y -i temp/txSignal.wav -ar 8000 -ab 4.75k temp/txSignal.amr');
[~,~] = system('ffmpeg -y -i temp/txSignal.amr -ar 8000 -ab 4.75k temp/txSignal_out.wav');
...
%correlation.
find =length(audio_data)-length(tx)-1;
amps = zeros(find,1);
len = length(tx);
for i=1:find
    amps(i) = sum(audio_data(i:i+len-1).*tx,"all");
end
[amp,max_id] = max(amps);
out = audio_data(max_id:max_id+len-1,1);
